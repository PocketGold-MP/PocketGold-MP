...
class Server implements Listener {
	...
	public function loadpgConfig(){
		...
		$this->mobAiEnabled = $this->getPocketGoldProperty("level.enable-mob-ai", false);
	}
	...
	public function getMotd() : string{
		return $this->getConfigString("motd", "PocketGold Server");
	}
	...
	public function getPocketGoldProperty(string $variable, $defaultValue = null){
		if(!array_key_exists($variable, $this->pgPropertyCache)){
			$this->pgPropertyCache[$variable] = $this->pgConfig->getNested($variable);
		}

		return $this->pgPropertyCache[$variable] ?? $defaultValue;
	}
	...
	public function __construct(\ClassLoader $autoloader, \AttachableThreadedLogger $logger, string $dataPath, string $pluginPath){
		...
		try{
			...
			if(!is_dir($dataPath . "extras/")) {

				mkdir($dataPath . "extras/", 0777);

			}

			if(!is_dir($dataPath . "extras/mute")) {

				mkdir($dataPath . "extras/mute", 0777);
				mkdir($dataPath . "extras/mute/players", 0777);
			...
			$this->pgConfig = new Config($this->dataPath . "settings.yml", Config::YAML, []);
			$this->loadpgConfig();

			$this->logger->info("Loading server properties...");
			$this->properties = new Config($this->dataPath . "server.properties", Config::PROPERTIES, [
				"motd" => "PocketGold Server",
				"server-port" => 19132,
				"white-list" => false,
				...
				" -=-=-=- Extras -=-=-=-",
				"onlyproxyjoin" => true,
				"onlyproxyjoin-message" => "§cPlease join the server over the proxy!",
				"mute" => true,
			]);
			...
			if($this->isAllowNether() and $this->getNetherLevel() === null){
				...
				$netherLevelName = $this->getPocketGoldProperty("dimensions.nether.level-name", "nether");
			...
			if($this->isAllowTheEnd() and $this->getTheEndLevel() === null){
				...
				$endLevelName = $this->getPocketGoldProperty("dimensions.the-end.level-name", "end");
	...
	public function isAllowNether() : bool{
		return (bool) $this->getPocketGoldProperty("dimensions.nether.active", true);
	}

	public function isAllowTheEnd() : bool{
		return (bool) $this->getPocketGoldProperty("dimensions.the-end.active", true);
	}
    ...
    private function getAdditionalPluginDirs() : array{
        return $this->getPocketGoldProperty("additional-plugin-dirs", []);
    }
