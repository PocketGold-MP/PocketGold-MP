...
class MainLogger extends \AttachableThreadedLogger{
	...
	public function __construct(string $logFile, bool $logDebug = false){
		...
		if(static::$logger instanceof MainLogger){
			throw new \RuntimeException("MainLogger wurde bereits erstellt");
		}
		touch($logFile);
	...
	public function logException(\Throwable $e, $trace = null){
		...
		$message = get_class($e) . ": \"$errstr\" ($errno) in \"$errfile\" in der Zeile $errline";
	...
	protected function send($message, $level, $prefix, $color){
		...
		$message = sprintf($this->format, "§7[§6" . $time->format("H:i:s") . "§7]§e", $color, $threadName, $prefix, TextFormat::clean($message, false));
		...
		$this->synchronized(function() use ($message, $level, $time) : void{
			...
			$this->logStream[] = $time->format("d-m-Y") . " " . TextFormat::clean($message) . PHP_EOL;
		});
	}
	...
	public function run(){
		...
		if(!is_resource($logResource)){
			throw new \RuntimeException("Protokolldatei konnte nicht geöffnet werden");
		}
